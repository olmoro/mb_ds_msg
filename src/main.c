...
//     queues->processing_queue = xQueueCreate(QUEUE_SIZE, sizeof(pdu_packet_t));


    
    // Создание задач
    BaseType_t modbus_receive_task_handle = xTaskCreate(modbus_receive_task, "modbus rx", 4096, NULL, 5, NULL);
    if (!modbus_receive_task_handle)
    {
        ESP_LOGE(TAG, "Failed to create Modbus Receive task");
        return;
    }
